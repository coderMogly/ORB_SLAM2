    ...
    mbInitTransformation(initTransformation), mbInitRequested(false), mInitTcw(cv::Mat::eye(4,4,CV_32F)), mInitScale(1.0)
{
    ...
    if (!fSettings.isOpened())
    	std::cerr << "Error: OrbSlamTracking failed to open settings" << std::endl;
...
{
    unique_lock<mutex> lock(mMutexInit);
    if(initTimeStamp) *initTimeStamp = mInitialFrame.mTimeStamp;
    if(currentTimeStamp) *currentTimeStamp = mCurrentFrame.mTimeStamp;
    return mbInitRequested;
}

void Tracking::SetInitTransformation(const cv::Mat &Tcw, const float &scale)
{
    unique_lock<mutex> lock(mMutexInit);
    mInitTcw = Tcw;
    mInitScale = scale;
